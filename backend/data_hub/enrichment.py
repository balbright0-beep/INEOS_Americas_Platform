...
def enrich(sap_df, handover_df=None, stock_pipeline_df=None,
           urban_science_df=None, ref_db_path='reference/reference.db'):
    """
    ...
    if handover_df is not None and 'customer_name' in handover_df.columns:
        ho_bill = handover_df[['vin', 'customer_name']].drop_duplicates('vin')
        ho_bill.columns = ['vin', 'bill_to_dealer']
        df = df.merge(ho_bill, on='vin', how='left')
        df['bill_to_dealer'] = df['bill_to_dealer'].fillna('Not Handed Over')
    else:
    ...
    try:
        ...
        df['msrp_less_vie'] = df.get('msrp', 0) - df['total_variable_spend']
    except Exception:
        df['incentive_spend'] = 0
        df['subvention_spend'] = 0
        df['total_variable_spend'] = 0
        df['msrp_less_vie'] = df.get('msrp', 0)
